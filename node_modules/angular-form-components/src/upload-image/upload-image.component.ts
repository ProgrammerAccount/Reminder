    ...
    ngOnInit() {
        ...
        if (this.limit) {
            if (this.limit.width && this.limit.height) {
                this.help += 'Image size: ' + this.limit.width + '*' + this.limit.height;
            }
            if (this.limit.size) {
                ...
                reader.onload = (e: ProgressEvent) => {
                    ...
                    image.onload = ev => {
                        ...
                        if (this.limit.size && file.size > this.limit.size * 1024) {
                            checkErr.push('Image should no more than ' + this.limit.size + ' K. ');
                        }
                        if ((this.limit.width && image.width > this.limit.width) ||
                            (this.limit.height && image.height > this.limit.height)) {
                            checkErr.push('Image size should be inner ' + (this.limit.width ? 'width: ' + this.limit.width + '. ' : '') +
                                (this.limit.height ? ' height: ' + this.limit.height + '. ' : ''));
                        }
